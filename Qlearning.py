...
class Qlearning(Agent):
    ...
    # use sequentially the random number in random_numbers.txt
    def get_random_num(self):
        num = float(self.file.readline().strip())
        if not num:
            self.file.seek(0)
            num = float(self.file.readline().strip())
        return num
    ...
    def select_action(self, S):
        ...
        if random_num < self.epsilon:
            # Choose the worst action for this state
            Qs = self.Q[S]
            minQ = min(Qs)
    ...
    def do_step(self, S, act, logfile=None):
        ...
        # else:
        #     # Choose the best action for this state
        #     # If two actions have the same Q value, pick one at Srandom
        #     Qs = self.Q[S]
        #     maxQ = max(Qs)
        #     As = list(where(Qs == maxQ)[0])
        #     A = random.choice(As)
